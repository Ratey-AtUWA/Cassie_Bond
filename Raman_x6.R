...
#  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
with(H_Cotton, plot(V2 ~ V1, type = "l", col = 2,
                    xaxt = "n",yaxt = "n",
...
mtext("Cotton", side = 3, line = -1.5, adj = 0.7, col = 2)
#  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
with(H_PTFE, plot(V2 ~ V1, type = "l", col = 3,
                  xaxt = "n", yaxt = "n", xlab="",
                  ylab = "", xlim = c(0,3150), ylim = c(0,5500)))
rect(-20,5000,100,par("usr")[4],lwd=0.01, border="white",col="white");box()
...
axis(1, labels=F)
axis(2, at=c(0,2000,4000), labels=c(0,2000,4000), cex.axis=1.4)
mtext("Polytetrafluoroethylene (PTFE)", side = 3,line = -1.5,adj = 0.7,col = 3)
#  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
with(K_PP, plot(V2 ~ V1, type = "l", col = 4,
                xaxt = "n", cex.axis=1.4,
                ylab = "", xlim = c(0,3150)))
axis(1, labels=F)
mtext("Polypropylene (PP)", side = 3, line = -1.5, adj = 0.7, col = 4)
#  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
with(K_Wool, plot(V2 ~ V1, type = "l", col = 5,
                  xaxt = "n", yaxt="n",
                  ylab = "", xlim = c(0,3150)))
axis(1, labels=F)
axis(2, at=c(0,200,400), labels=c(0,200,400), cex.axis=1.4)
mtext("Wool", side = 3, line = -1.5, adj = 0.7, col = 5)
mtext("Intensity", 2, 3, cex = 1.2, adj=1, font=2, las=0)
#  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
with(SB_PVDF, plot(V2 ~ V1, type = "l", col = 6,
                   xaxt = "n", cex.axis=1.4,
                   ylab = "", xlim = c(0,3150)))
axis(1, labels=F)
mtext("Polyvinylidene fluoride (PVDF)", side = 3, line = -1.5, adj = 0.7, col = 6)
#  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
with(SC_PE, plot(V2 ~ V1, type = "l", col = 7,
                 xlab = "", cex.axis=1.4,
                 ylab = "", xlim = c(0,3150)))
mtext(expression(bold(paste("Relative wavenumbers (",cm^"-1",")"))), 1, 2.2)
mtext("Polyethylene (PE)", side = 3, line = -1.5, adj = 0.7, col = 7)
#  -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
